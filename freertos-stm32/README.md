...
**Status: builds and runs stably on real hardware (Nucleo-F411RE, ST-Link V2.1), including the full MPU-trap demo end to end. The core system (scheduler, UART reporting, ADC sensor, hardware watchdog) has been verified over continuous runs of 60+ seconds with zero faults, and the "VIOLATE" command now triggers a real, caught MemManage fault — `MMFAR` matches `g_secure_secret`'s address exactly, cross-checked against the linked ELF's own symbol table — after which the board resets cleanly and resumes stable operation.**
...
10. **Unprivileged-callable syscall gateway placed behind the privileged-only wall**: `.freertos_system_calls` (the `MPU_xxx` wrapper functions unprivileged tasks must be able to branch into to reach the `svc` instruction that raises their privilege — see `vSVCHandler_C` in `port.c`) was inside the same "privileged-only, execute-denied-to-everyone-else" flash carve-out as the real kernel implementation. Self-defeating by construction: unprivileged code can't fetch the wrapper's first instruction, so it can never reach the `svc` that would let it in. Confirmed via a real `IACCVIOL` fault the moment the (previously-starved, see bug #11) "Untrusted" task got to run and called `vTaskDelay()`. Fixed by moving `.freertos_system_calls` to after the privileged-only carve-out, where it's covered by the general "unprivileged R+X across all of flash" region instead — verified via `objdump -h` that it now sits outside `[__privileged_functions_start__, __privileged_functions_end__)`.
11. **Custom stack canary collided with FreeRTOS's own native overflow check**: with bug #10 fixed and the "Untrusted" task's priority raised so it could finally run (it was starved at priority 1 by UART/Sensor's busy-polling, same root cause as bug #5), it hit a *third* wall: `vApplicationStackOverflowHook()` fired for `"Untrusted"` the instant it was first switched away from — a silent `NVIC_SystemReset()`, no fault message, so it looked completely different from every other bug here. Growing `UNTRUSTED_STACK_WORDS` (128 → 256 → 512) changed nothing, which was already a sign this wasn't a real overflow. GDB confirmed it directly: `pxCurrentTCB->pxStack` correctly pointed at `untrusted_stack`, and words 1-3 of it still held FreeRTOS's own `0xa5a5a5a5` fill pattern — but word 0 held `0xDEADBEEF`. That's this project's own stack canary (`Security_RegisterTask()`, see bug #4), planted at `untrusted_stack[0]` — the *exact same word* `stack_macros.h`'s native `configCHECK_FOR_STACK_OVERFLOW == 2` check inspects (it compares words 0-3 against `0xa5a5a5a5` on every context switch). Two independent, individually-correct integrity mechanisms sharing one word of memory, each blind to the other. Fixed by moving the canary to `untrusted_stack[4]`, outside the range FreeRTOS's own check looks at.
...
## VIOLATE demo — real captured output

```
[UART] arming deliberate MPU violation on Untrusted task...
...
`MMFAR=0x20008000` matches `g_secure_secret`'s address exactly (`arm-none-eabi-nm`), confirming the fault was raised on that specific illegal write, not some unrelated corruption. The board resets once (by design — `report_and_reset()` in `fault_handlers.c`) and resumes normal, stable operation immediately afterward.

## Known limitations

- Not yet tested: the HMAC-SHA256 UART challenge-response protocol under real sustained load.
